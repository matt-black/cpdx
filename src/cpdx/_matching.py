...
def expectation_weighted(
    ...
    Notes:
        Target point weights, `alpha_n`, do not contribute to the matching b/c we assume that the weightings do not affect the probability of a point (not) being an outlier.
    """
    n, d = x.shape
    m, _ = y_t.shape
    d_t = sqdist(x, y_t).transpose()  # (m, n)
    top = pi_m * jnp.exp(jnp.negative(jnp.divide(d_t, 2 * var)))
    outl_term = jnp.divide(w, 1.0 - w) * jnp.divide(
        jnp.float_power(2 * jnp.pi * var, d / 2) * m, n
    )
    bot = jnp.add(
